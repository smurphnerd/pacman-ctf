...
class MixedAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState: GameState):
        ...
        if not self.posSatisfyLowLevelPlan(gameState):
            self.lowLevelPlan = self.getLowLevelPlanHS(gameState, "win")
            self.lowLevelActionIndex = 0
    ...
    def getLowLevelPlanHS(
        ...
        if highLevelAction == "win":
            ...
            for action in gameState.getLegalActions(self.index):
                ...
                is_scared_ghost = (
                    successor.getAgentState(self.index).scaredTimer > 0
                    and not is_pacman
                )
                ...
                for capsule in self.getCapsulesYouAreDefending():
                    positive_count += int(
                        self.get_advantage(capsule, gameState, advantages) >= 0
                    ) * min(40, len(self.getFoodYouAreDefending()))
            ...
            if len(best_next) > 1:
                # Filter out best next that are in same position as ally
                best_next = [
                    (action, next_pos)
                    for action, next_pos in best_next
                    if next_pos != gameState.getAgentPosition((self.index + 2) % 4)
                ]
                ...
                if len(enemy_poss) == 0:
                    # Choose the one that maximizes disntance to ally
                    prev_dist = self.getMazeDistance(
                    ...
                    best_next = sorted(
                        ...
                        key=lambda x: self.getMazeDistance(
                            x[1], gameState.getAgentPosition((self.index + 2) % 4)
                        ),
                    )
                    actual_best = best_next[-1]
                    actual_best_dist = self.getMazeDistance(
                        actual_best[1], gameState.getAgentPosition((self.index + 2) % 4)
                    )
                    if prev_dist == actual_best_dist:
            ...
            enemy_scared = tuple(
                gameState.getAgentState(i).scaredTimer > 4
                for i in self.getOpponents(gameState)
                if gameState.getAgentPosition(i) is not None
            )
                ...
                for action, next_pos, successor, advantages in trapped_actions:
                    ...
                    if (
                        min(
                            (
                                self.getMazeDistance(next_pos, e_pos)
                                for i, e_pos in enumerate(def_enemy_pos)
                                if not enemy_scared[i]
                            ),
                            default=999,
                        )
                        <= 1
            ...
            max_positive_count = -1
            next_actions = []
            best_next = []  # Initialize to prevent unbound variable
            def_enemy_pos = tuple(
                gameState.getAgentPosition(i)
                for i in self.getOpponents(gameState)
                if gameState.getAgentPosition(i) is not None
            ...
            for action in legalActions:
                ...
                trapped = trapped and successor.getAgentState(self.index).isPacman
                ...
                threatened = threatened and self.isInEnemyTerritory(next_pos)
                threatened = (
                    threatened
                    or self.getMazeDistance(
                        gameState.getAgentPosition(self.index), next_pos
                    )
                    > 1
                )
            ...
            if len(next_actions) == 0:  # trapped. try to run
                ...
                for action, next_pos, successor, advantages in trapped_actions:
                    ...
                    if adv > max_adv:
                        max_adv = adv
                        best_next = [(action, next_pos)]
                if max_adv == -999 and die_acts:
                    best_next = [die_acts[0]]

            elif len(next_actions) > 1:
                # Filter out best next that are in same position as ally
                next_actions = [
                    (action, next_pos, successor, advantages, pos_count, ate)
            ...
            for action in legalActions:
                ...
                if successor.getAgentState(self.index).isPacman and trapped:
                    trapped_actions.append((action, next_pos, min_enemy_adv))
                    continue
    ...
    def calculate_advantages(self, gameState: GameState):
        ...
        # Store a list of the distance each agent is from all escape points they are defending
        distance_to_escapes_defending = defaultdict(list)
        for idx in range(4):
            if idx in self.getTeam(gameState):
                for escape in self.getEscapePointsYouAreDefending():
                    distance_to_escapes_defending[idx].append(
                        self.getMazeDistance(
                            gameState.getAgentPosition(idx),
                            escape,
                        )
                    )
            else:
                for escape in self.getEscapePoints():
                    distance = min(
                        self.getMazeDistance(pos, escape) for pos in enemy_pos[idx]
                    )
                    distance_to_escapes[idx].append(distance)
        ...
        food_tiles = set(self.getFood() + self.getFoodYouAreDefending())
                ...
                if not walls[x][y] and (x, y) in tiles:
                    ...
                    for idx in range(4):
                        if idx in self.getTeam(gameState):
                            ...
                            # Basically here, for each food cell, I also want to add the distance to each escape point
                            # This way, when we calculate advantages, we can just consider whether we can still intercept
                            # the enemy in time before they escape with the food
                            if node_pos in food_tiles:
                                distances = []
                                # Add the distance to each exit position if this is a food cell
                                if self.isInEnemyTerritory(node_pos):
                                    exit_positions = self.getEscapePoints()
                                    for ext in exit_positions:
                                        distances.append(
                                            self.getMazeDistance(ext, node_pos)
                                            + distance
                                        )
                                # Just use the direct distance (unless scared timer is larger)
                                else:
                                    if gameState.getAgentState(
                                        idx
                                    ).scaredTimer > distance and self.isInHome(
                                        node_pos
                                    ):
                                        distance = gameState.getAgentState(
                                            idx
                                        ).scaredTimer
                                    distances = distance_to_escapes_defending[idx]
                            # Just use the direct distance (TODO: maybe add scred timer logic here too)
                            else:
                                distances = [distance]

                            advantages[node_pos][idx] = (distance, distances)

                        # Same logic as above, but for opponents, using the minimum distance from the beliefs
                        else:
                            distance = min(
                            ...
                            if node_pos in food_tiles:
                                distances = []
                                if self.isInHome(node_pos):
                                    exit_positions = (
                                        self.getEscapePointsYouAreDefending()
                                    )
                                    for ext in exit_positions:
                                        distances.append(
                                            self.getMazeDistance(node_pos, ext)
                                            + distance
                                        )
                                else:
                                    if gameState.getAgentState(
    ...
    def get_advantage(self, pos, gameState, advantages, teammate=None, enemy=None):
        ...
        # Closest opponent and teammate
        min_op = min(enemy, key=lambda x: advantages[pos][x][0])
        min_team = min(teammate, key=lambda x: advantages[pos][x][0])

        # For food cells, the zipped lists should be the corresponding escape points
        # For non-food cells, the zipped lists will just be the single distance
        # TODO: idk if this is correct
        adv = min(
            op_dist - team_dist
            for op_dist, team_dist in zip(
                advantages[pos][min_op][1], advantages[pos][min_team][1]
            )
        )
