...
class RewardWeights:
    ...
    time_penalty: float
    # Dense shaping rewards (can be set to 0 to disable)
    distance_to_food: float
    crossed_to_enemy: float
    returned_home: float
...
class SmurphAgent(CaptureAgent):
    ...
    def _calculate_reward(
        ...
        # ----- Dense Shaping Rewards (can be disabled by setting weights to 0) -----

        # Reward for crossing into enemy territory
        if not my_prev.isPacman and my_curr.isPacman:
            reward += w.crossed_to_enemy

        # Reward for returning home as ghost
        if my_prev.isPacman and not my_curr.isPacman and my_curr_pos != my_curr.start.pos:
            reward += w.returned_home

        # Distance-based reward when seeking food
        if my_curr.isPacman:
            food_list = self.getFood(current_gameState).asList()
            if food_list:
                min_dist = min([self.getMazeDistance(my_curr_pos, food) for food in food_list])
                # Inverse distance reward (closer = better)
                reward += w.distance_to_food / (min_dist + 1)
